...
class Host(Base):
    ...
    jump: Mapped[str | None] = mapped_column(String, nullable=True) # Jump host alias
    ssh_key: Mapped[str | None] = mapped_column(String, nullable=True)
...
class AuditSession(Base):
    ...
    input_mode: Mapped[str | None] = mapped_column(String(32), nullable=True)  # 输入模式
    final_output: Mapped[str | None] = mapped_column(Text, nullable=True)  # 最终输出

    # Agent 推理
    agent_reasoning: Mapped[str | None] = mapped_column(Text, nullable=True)  # Agent 推理过程
    provider: Mapped[str | None] = mapped_column(String(32), nullable=True)  # 使用的 LLM Provider

    # 状态
    status: Mapped[str] = mapped_column(String(32))  # completed / failed / cancelled

    # 资源使用
    token_usage: Mapped[dict | None] = mapped_column(JSON, nullable=True)  # {input, output, total}
    total_duration_sec: Mapped[float | None] = mapped_column(Float, nullable=True)  # 总耗时
    cost_usd: Mapped[float | None] = mapped_column(Float, nullable=True)  # 成本（美元）
...
class AuditToolCall(Base):
    ...
    user_confirmed: Mapped[int | None] = mapped_column(Integer, nullable=True)
    confirm_time: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)

    # 执行结果
    execution_start: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    execution_end: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
