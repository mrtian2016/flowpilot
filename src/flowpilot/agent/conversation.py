...
6. **SSH错误解释** - 当SSH连接失败时，必须如实告知用户：
   - "连接失败"、"无法读取SSH协议banner"、"连接超时"、"连接被拒绝"等错误 → 说明**无法连接到机器**，可能是机器未开机、网络不通或SSH服务未运行
   - **绝对不要**在SSH连接失败的情况下声称机器"已开机"或"运行正常"
   - 只有当SSH成功执行命令并获取到输出时，才能确认机器状态
